Seata 的核心角色
Seata 的架构里有 3 个最重要的角色：
 TC：Transaction Coordinator，事务协调者
负责维护全局事务和分支事务的状态，决定全局提交还是全局回滚。可以把它理解为“总调度中心”。
GitHub
 TM：Transaction Manager，事务管理器
负责定义全局事务边界，比如开启一个全局事务、提交或回滚全局事务。通常你在业务代码上加 @GlobalTransactional，背后主要就是 TM 在发挥作用。
GitHub
 RM：Resource Manager，资源管理器
负责管理分支事务对应的资源，比如数据库连接、数据源代理、本地事务提交与回滚，以及向 TC 注册分支事务。
...
5. AT 模式：最常用、上手最快
5.1 AT 是什么
AT 可以理解为 Automatic Transaction。它主要用于关系型数据库场景，对业务代码侵入相对较低，是 Seata 最常见的入门模式。�
GitHub
5.2 它怎么工作
AT 模式下，Seata 会代理你的数据源，在本地事务执行前后记录必要信息，典型包括：
业务 SQL 执行前的镜像（before image）
业务 SQL 执行后的镜像（after image）
当全局事务需要回滚时，Seata 根据这些镜像生成反向补偿，把数据恢复到事务前状态。这个过程本质上是“基于 undo log 的自动回滚”。这一点是 AT 最核心的机制。Seata samples 和官方 quickstart 也长期以 AT 作为最常见演示模式。�
GitHub +1
5.3 AT 的优点
对业务侵入小
对 Spring Boot / MyBatis / JDBC 这类传统 Java 服务比较友好
开发成本低，适合大多数“数据库更新型”业务
5.4 AT 的局限
更适合关系型数据库
对 SQL 和数据源代理有依赖
对复杂 SQL、非标准更新、跨非数据库资源不如 TCC 灵活
一致性很强，但不是传统数据库单机事务那种“无感透明”
5.5 适用场景
电商下单
库存扣减
余额扣减
订单状态流转
以数据库更新为主的中后台业务
如果你的系统主要是 Spring Boot + MySQL/PostgreSQL + MyBatis/JPA，这通常是首选。
6. TCC 模式：可控性最强，但开发成本高
6.1 TCC 是什么
...
Confirm：正式提交
Cancel：取消预留并回滚
这是一种业务层面的分布式事务，不再依赖数据库自动回滚，而是让你自己显式写提交和补偿逻辑。�
GitHub +1
6.2 TCC 的本质
TCC 并不是“数据库帮你回滚”，而是：
你先把资源冻结住
...
对业务补偿逻辑要求清晰的金融类流程
跨资源协调，不只是数据库更新
你之前一直在问 Seata 和 Spring Integration、WebClient、TCC 的配合，本质上就是因为 TCC 更强调业务动作的显式建模，不像 AT 那样主要依赖数据源代理。
7. XA 模式：标准两阶段提交
7.1 XA 是什么
XA 是基于数据库 XA 协议的两阶段提交模式。Seata 也支持 XA。�
GitHub +1
7.2 XA 的思路
XA 依赖数据库自身支持分布式事务协议，整体是经典 2PC：
第一阶段：prepare
...
8. Saga 模式：长事务、最终一致性
8.1 Saga 是什么
Saga 面向的是长事务和最终一致性场景。它的思想不是把所有参与者锁在一个强一致事务里，而是把整个业务拆成多个本地事务，每一步都配一个补偿动作。某一步失败时，就按相反顺序执行补偿。Seata 支持 Saga，并且近期版本中对 Saga 注解模式也有调整，例如在 2.6.0 变更说明里提到 saga annotation pattern 的相关替换。�
GitHub +1
8.2 Saga 的特点
不追求实时强一致
更强调最终一致性
...
对业务流程可控性要求高
就考虑 TCC。
9.3 数据库原生 XA 环境成熟，用 XA
当你所在环境：
数据库对 XA 支持好
更偏传统企业事务方案
能接受 2PC 的性能与锁影响
可以考虑 XA。
9.4 长事务、流程编排、最终一致性，用 Saga
当你的业务：
事务持续时间长
步骤多
涉及外部系统
...
以最常见的全局事务为例：
第一步：TM 开启全局事务
业务入口方法加 @GlobalTransactional，TM 向 TC 申请开启一个全局事务，拿到全局事务 ID，也就是 XID。�
GitHub
第二步：XID 传播
这个 XID 会沿着调用链传递给下游服务。下游服务收到后，知道自己是这个全局事务的一部分。
第三步：RM 注册分支事务
每个参与的服务在执行本地事务前，会向 TC 注册一个分支事务。
第四步：本地事务执行
每个服务执行自己的数据库操作或业务动作。
第五步：全局提交或回滚
如果所有分支都成功，TM 通知 TC 提交全局事务
如果任一分支失败，TM/TC 触发全局回滚
第六步：各分支完成提交或补偿
不同模式下的具体回滚机制不同：
AT：根据 undo log 回滚
TCC：执行 Cancel
...
Spring Integration 负责的是**流程编排、消息分发、聚合、线程模型、错误流转**，它自己不是分布式事务协调器。
你可以先这样记：
